...
export class Editor {
  ...
  update(newState, action) {
    ...
    if (action instanceof model.QueuePattern) {
      ...
      node.queuePattern(action.patIdx);
    ...
    if (action instanceof model.SetPattern) {
      let nodeState = newState.nodes[action.nodeId];
      node.setPattern(action.patIdx, nodeState);
    ...
    for (let nodeId in newState.nodes) {
      ...
      let nodeClass =
        nodeState.type in NODE_CLASSES ? NODE_CLASSES[nodeState.type] : UINode;
    ...
    for (let dstId in newState.nodes) {
      ...
      for (let dstPort in dstState.ins) {
        if (!dstState.ins[dstPort]) continue;
    ...
    this.selected = this.selected.filter((nodeId) => this.nodes.has(nodeId));
  ...
  updateSelect(startPos, curPos) {
    ...
    for (let [nodeId, node] of this.nodes) {
      ...
      let nodeInside =
        left > xMin && left + width < xMax && top > yMin && top + height < yMax;
  ...
  resize() {
    ...
    this.graphDiv.style.width = `${maxWidth}px`;
    this.graphDiv.style.height = `${maxHeight}px`;
  }
  ...
  createNodeDialog(mousePos) {
    ...
    for (let nodeType in NODE_SCHEMA) {
      ...
      function subDivClick(evt) {
        ...
        this.model.update(
          new model.CreateNode(nodeType, mousePos.x, mousePos.y)
        );
      }
  ...
  endDrag(mousePos) {
    ...
    if (dx != 0 || dy != 0) {
      this.model.update(new model.MoveNodes(this.selected, dx, dy));
    }
  ...
  deleteSelected() {
    ...
    this.model.update(new model.DeleteNodes(this.selected));
  }
  ...
  groupSelected() {
    ...
    this.model.update(new model.GroupNodes(this.selected));
  }
}
...
class Edge {
  ...
  get openSide() {
    if (this.srcNode === null) return "src";
    else if (this.dstNode === null) return "dst";
    return null;
  }
...
class UINode {
  ...
  genNodeDOM(state) {
    ...
    for (var portIdx = 0; portIdx < this.numIns; ++portIdx) {
      this.genPortDOM(inPortsDiv, portIdx, state.inNames[portIdx], "dst");
    }

    // Create the source (output) ports
    for (var portIdx = 0; portIdx < this.numOuts; ++portIdx) {
      this.genPortDOM(outPortsDiv, portIdx, state.outNames[portIdx], "src");
    }
  }
    ...
    function portClick(action) {
      ...
      if (!editor.edge) {
        ...
        if (side == "dst") {
          ...
          editor.model.update(new model.Disconnect(this.nodeId, portIdx));
  ...
  paramsDialog() {
    ...
    function saveParams() {
      ...
      this.send(new model.SetNodeName(this.nodeId, newName));

      // For each parameter
      for (let param of this.schema.params) {
        if (newParams[param.name] == nodeState.params[param.name]) continue;

        this.send(
          new model.SetParam(this.nodeId, param.name, newParams[param.name])
        );
      }
  ...
  getPortPos(portIdx, side) {
    let connDiv =
      side == "dst" ? this.inPorts[portIdx] : this.outPorts[portIdx];
...
class ClockDiv extends UINode {
  constructor(id, state, editor) {
    ...
    function factorChange() {
      ...
      this.send(new model.SetParam(this.nodeId, "factor", factor));
    }
  ...
  constructor(id, state, editor) {
    ...
    input.onchange = (evt) => {
      ...
      if (input.value.trim() === "" || isNaN(val)) {
        ...
        editor.model.update(new model.SetParam(id, "value", val));
      }
...
class KnobNode extends UINode {
  constructor(id, state, editor) {
    ...
    function knobChange(value) {
      editor.model.update(new model.SetParam(id, "value", value));
    }
...
class Sequencer extends UINode {
  constructor(id, state, editor) {
    ...
    shrinkBtn.onclick = (evt) =>
      this.send(new model.ShrinkPattern(this.nodeId));
    ...
    extendBtn.onclick = (evt) =>
      this.send(new model.ExtendPattern(this.nodeId));
    ...
    minusBtn.onclick = (evt) =>
      this.send(new model.ShrinkPattern(this.nodeId, 1));
    ...
    plusBtn.onclick = (evt) =>
      this.send(new model.ExtendPattern(this.nodeId, 1));
  ...
  genGridDOM(patIdx, state) {
    ...
    if (this.patDivs[patIdx]) this.gridDiv.removeChild(this.patDivs[patIdx]);
  ...
  queuePattern(patIdx) {
    ...
    function blink(state) {
      this.patBtns[patIdx].className = state
        ? "patsel_btn_queue"
        : "patsel_btn";
...
class MonoSeq extends Sequencer {
  ...
  setGridCell(patIdx, stepIdx, rowIdx, value) {
    ...
    for (let i = 0; i < row.length; ++i) row[i].className = "cell_off";
...
class Notes extends UINode {
  constructor(id, state, editor) {
    ...
    function oninput(evt) {
      this.send(
        new model.SetParam(this.nodeId, "text", textArea.value.trimEnd())
      );
    }
...
class Scope extends UINode {
  constructor(id, state, editor) {
    ...
    this.redraw(state.params.minVal, state.params.maxVal, state.samples);
  }
